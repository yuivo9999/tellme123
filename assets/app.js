...
4. 必须使用本章 requiredEntities 中的全部实体，不得自造新人名/地名/专名。
5. 人物言行须符合其性格设定；对话须有辨识度；时间线须与上一章衔接。
6. 若 L0 风格契约有量化指标，请在输出前自检：平均句长、对话占比是否落在契约区间内。
7. 结尾须指向下一章标题，埋下线索或悬念，但不得提前揭示下一章具体情节。
...
const IDEA_POLISH_SYS_LEGACY = `你是一位深谙长篇网文与影视叙事的构想编辑。你的任务是把用户的粗糙故事构想，优化成一份**结构化、可执行、可校验**的高质量构想。

【输出格式】
必须严格只输出如下 JSON（不要解释、不要 markdown 代码块）：
{
  "optimizedIdea": "150-300 字的条目式优化构想（保留用户原意，补全可推导细节）",
  "defects": ["缺陷1：用户构想中缺少xxx", "缺陷2：xxx设定不够具体"],
  "navBeacon": {
    "genre": "题材（如重生复仇/悬疑解谜/末世生存）",
    "protagonist": "主角：身份+核心动机+特殊能力/缺陷",
    "antagonist": "对手/阻力：身份+与主角的冲突来源",
    "coreConflict": "核心冲突：一句话概括主角与谁的不可调和矛盾",
    "worldRules": ["世界观规则1", "世界观规则2"],
    "tone": "风格基调（冷峻/热血/温情/悬疑/黑暗/治愈）",
    "targetExperience": "目标读者体验（如追更爽感/沉浸解谜/情绪共鸣）"
  },
  "seedCharacters": [
    {"name":"人名", "identity":"身份", "age":"岁数", "gender":"性别", "appearance":"外貌", "hobby":"爱好", "relation":"关系", "trait":"性格"}
  ],
  "seedPlaces": [
    {"name":"地名", "type":"类型", "note":"设定要点"}
  ],
  "suggestedChapterCount": 30
}

【硬性约束】
1. 绝不删减、篡改用户明确表达的内容（题材/元素/风格）。
2. 不替用户新增核心设定（不凭空加角色/势力/金手指），只补全"可推导的通用细节"。
3. 输入极短（少于 15 字）时：navBeacon、seedCharacters、seedPlaces 按题材惯例生成通用骨架，并在 optimizedIdea 开头标注"（基于题材惯例的通用展开）"，defects 列出必须补充的内容。
4. seedCharacters 每个人物必须输出完整 7 字段：identity/age/gender/appearance/hobby/relation/trait，缺字段填"未知"。
5. 若用户构想含风格基调，navBeacon.tone 必须与之完全一致，并在 optimizedIdea 中给出 2-3 个落地方式。
6. defects 必须真实指出用户构想中可被后续 AI 利用的信息缺口，不得为空（至少 1 条）。
7. suggestedChapterCount 必须是 5-200 之间的整数。`;

// 4.7 Pro（3.1）优化构想 AI 新系统提示词：资深长篇策划编辑 + 故事诊断师，输出结构化故事简报（含缺陷清单）
const IDEA_POLISH_SYS_PRO = `你是一位资深长篇小说策划编辑，同时担任「故事诊断师」。
【核心任务】把用户输入的粗糙故事构想，诊断并优化成一份可直接交付给大纲 AI 的「结构化故事简报」。

【处理流程】
1. 先诊断：识别构想中是否缺失下列关键要素，缺失几项就列出几项（不要硬造内容）。
2. 再优化：在保留用户原意的前提下，把构想补全为结构化简报。

【必须输出的 JSON 结构】
{
  "diagnosis": {
    "missing": ["缺失要素1", "缺失要素2"],  // 从下面 8 项中勾选：题材定位、主角、核心冲突、世界观/规则、核心对手/压力、主角动机、风格基调、目标读者体验
    "questions": ["为了补全题材定位，请回答：故事发生在什么时代/世界？", "..."]  // 每个缺失项对应一个引导问题
  },
  "brief": {
    "genre": "题材定位（如：都市悬疑 / 古风权谋 / 末世生存）",
    "protagonist": "主角身份 + 核心困境（≤40字）",
    "coreConflict": "核心冲突：谁想要什么？阻碍是什么？代价是什么？（≤80字）",
    "worldOrRules": "世界观或核心规则（没有就写"无"，绝不硬造）",
    "antagonistOrPressure": "对手/压力源（没有就写"无"）",
    "motivation": "主角行动的内在动机",
    "style": "风格基调 + 2-3 个落地方式",
    "readerExperience": "目标读者读完后的情绪/体验"
  },
  "advice": "给用户的下一步建议（≤60字）"
}

【硬性约束】
1. diagnosis.missing 只能从 8 个枚举值中选取：题材定位、主角、核心冲突、世界观/规则、核心对手/压力、主角动机、风格基调、目标读者体验。禁止自创缺失项名称。
2. 绝不删减、篡改用户明确表达的内容；只能在原意上细化。
3. 不替用户新增故事设定（不凭空加角色/势力/冲突/金手指），只补全"可推导的通用细节"。
4. brief.worldOrRules / antagonistOrPressure 若用户未提及，必须写"无"，禁止硬造。
5. 极短输入（<15 字）时：diagnosis.missing 全部勾选，brief 各字段按题材惯例给出通用模板，并在 advice 中明确告知用户"这是基于题材惯例的通用展开，建议补充具体设定"。
6. 只输出上述 JSON，不要 markdown 代码块、不要解释文字。

【输出示例】
{
  "diagnosis": {"missing":["世界观/规则"], "questions":["这个世界的运行规则或特殊设定是什么？"] },
  "brief": {
    "genre": "都市悬疑",
    "protagonist": "落魄前刑警因女儿失踪案被迫重出江湖",
    "coreConflict": "他必须在 72 小时内找到女儿，而绑匪似乎来自他已故搭档的过去",
    "worldOrRules": "无",
    "antagonistOrPressure": "绑匪 + 警局内部阻力",
    "motivation": "救女儿、赎罪",
    "style": "冷峻克制，短句为主，克制抒情，用细节替代解释",
    "readerExperience": "紧张、揪心、结尾反转后的沉重"
  },
  "advice": "建议补充世界观：是普通当代都市，还是有特殊社会背景？"
}`;
...
要求：输出 2-6 个方案；每个方案都是完整 JSON；方案差异只在走向/补全，都必须保留用户原意；navBeacon 中的 genre/coreConflict/tone 必须一致，只允许 protagonist/antagonist/suggestedChapterCount 等可变。`;
...
3. 首次新建某副线时，question 必填；若给不出核心问题，则不输出该副线。
...
async function regenAllTitles(btn){
  ...
  // 4.8 旗舰版（P1）：拓扑路由检查 + 运行态标记
  if(!canRunAI('titles')){ toast('请先完成上游步骤：生成大纲'); return; }
  markAIRunning('titles');
    ...
    else { addToFixQueue({kind:'titles', error:e.message}); toast('重生成标题失败：'+e.message); }
  }
  finally{ state.aiNetwork.running = (state.aiNetwork.running||[]).filter(k=>k!=='titles'); hideStopBtn(); if(preview) preview.remove(); if(btn) busy(btn,false); }
}
...
async function genAllTitles(btn){
  ...
  try{
    ...
    else { addToFixQueue({kind:'titles', error:e.message}); toast('生成标题失败：'+e.message); }
  }
  finally{ state.aiNetwork.running = (state.aiNetwork.running||[]).filter(k=>k!=='titles'); hideStopBtn(); if(preview) preview.remove(); if(btn) busy(btn,false); }
}
...
function openReader(i){
  ...
  if(csum) fallback = `<p class="muted">🗂 本章梗概：${esc(csum)}</p>
    ...
    <p class="muted" style="margin-top:6px">生成正文后将在此展示全文。可用下方「重生成」或「一键批量生成」补写。</p>`;
  // 4.6 Plus（2.7）：阅读器顶部插入本章相关词典浮板（方案 A）
  const rg = relevantGlossaryForChapter(i);
  const rgHtml = formatRelevantGlossaryHtml(rg);
  $('#readerBody').innerHTML = (rgHtml || '') + (paras.length ? paras.map(p=>`<p>${esc(p)}</p>`).join('') : fallback);
  // 构建目录并定位当前章
  renderToc(i);
  readerCur = i;
...
async function genChapterPlans(btn){
  ...
  if(!isLong() || !o) return;
  // 4.8 旗舰版（P1）：拓扑路由检查 + 运行态标记
  if(!canRunAI('chapterPlan')){ toast('请先完成上游步骤：生成大纲与标题'); return; }
  ...
  try{
    ...
    if(!totalN){ toast('请先生成章节标题'); return; }
    const batches = [];
    for(let start=0; start<totalN; start+=PLAN_BATCH_SIZE){
      batches.push({start, end: Math.min(start+PLAN_BATCH_SIZE, totalN)});
    }
    let allPlans = new Array(totalN).fill(null);
    let allTitles = [];
    let seeded = 0;
    for(const [bi, b] of batches.entries()){
      ...
      // 前文骨架：前批的标题+末章简述
      const prevSkeleton = b.start > 0 ? buildPrevSkeleton(b.start) : '';
      const parts = [];
      ...
      // 4.7 Pro（3.4）：批次 user 统一注入整体情绪基调
      parts.push(`【整体情绪基调】${o.tone || '未指定'}`);
      ...
      // 合并 chapterPlans
      (j.chapterPlans||[]).forEach((p, i)=>{ allPlans[b.start + i] = p; });
      // 第一批 glossary
      if(bi === 0 && j.glossary) seeded = mergeSeedGlossary(j.glossary);
      // 保存进度
      persist();
    }
    // 校验全部
    assertCount(allPlans, totalN, 'chapterPlans');
    assertCount(allTitles, totalN, 'titles');
    // 4.7 Pro（3.4）：合并后全量整体校验（summary / beats 四段 / beat 类型顺序）
    const _cpRep = validateChapterPlanOutput({titles: allTitles, chapterPlans: allPlans});
    if(!_cpRep.ok) throw new Error(`规划师输出校验失败：${_cpRep.code} ${_cpRep.details||''}`);
    // 覆盖前快照
    pushChapterPlansSnapshot();
    o.chapterPlans = allPlans;
    // 应用 titles
    if(bindPlannerTitles(allTitles)){
      state.plannerFinalized = true;
    }
    persist();
    // 从 plans 提取新实体
    const planTexts = allPlans.map(p=> p && p.summary).filter(Boolean);
    if(planTexts.length && state.glossAutoFill && sourceHasGlossary(o.glossary)){
      const ext = await extractNewGlossary(planTexts);
      const nm = mergeExtractedGlossary(ext);
      if(nm.total > 0) persist();
    }
    render();
    markAIDone('chapterPlan');   // 4.8（6.4）：成功后标记完成
    toast(`已生成 ${allPlans.filter(Boolean).length} 条主线简述 · 节拍表已注入`);
  }catch(e){
    if(e.name==='AbortError') toast('已停止生成梗概');
    else { addToFixQueue({kind:'chapterPlan', error:e.message}); toast('梗概生成失败：'+e.message); }
  }finally{ state.aiNetwork.running = (state.aiNetwork.running||[]).filter(k=>k!=='chapterPlan'); hideStopBtn(); if(preview) preview.remove(); if(btn){ btn.classList.remove('cp-gen-btn-loading'); busy(btn,false); } }
}

// 4.5：前文骨架（供规划师批间衔接）：全部前序标题 + 最近 3 章简述
function buildPrevSkeleton(endIdx){
  const o = state.outline;
  const titles = (o.chapters||[]).slice(0, endIdx).map((c,i)=> `第${i+1}章《${c&&c.title||''}》`).join(' / ');
...
async function validateChapterContent(i, text){
  ...
  // 1. 字数
  if(w < 1400 || w > 7000){
    report.ok = false; report.code = AI_ERR.WORD_OVER;
    report.errors.push(`字数 ${w} 超出 1400-7000 安全区间`);
  }
  // 2. 节拍表覆盖
  if(plan && Array.isArray(plan.beats)){
  ...
  ['characters','places','propernouns'].forEach(k => (g[k]||[]).forEach(it => knownNames.add(String(it.name||'').trim())));
...
function detectUnknownProperNouns(text, knownNames){
  ...
  const m = text.match(/[\u4e00-\u9fa5]{2,6}/g) || [];
  const freq = {};
  m.forEach(w => { if(!common.has(w)) freq[w] = (freq[w]||0)+1; });
  return Object.keys(freq).filter(w => freq[w] >= 2 && !knownNames.has(w)).slice(0, 5);
}
...
function dialogueRatio(text){
  ...
  const total = (text.match(/[\u4e00-\u9fa5]/g) || []).length;
  if(!total) return 0;
  // 粗略估算：每段对话约 30 字，引号成对出现
...
async function genNChapters(start, n){
  ...
  for(let k=0; k<n; k++) finalizeChapterTitle(start+k);
  // 4.5：批次完成后异步补全滚动摘要（静默失败，不阻塞批次流程）
  generateRollingSummaries().catch(()=>{});
  }finally{
