...
/* ---------- 全局状态 ---------- */
const APP_VERSION = '1.0.192';   // v1.0.192 铁律精简审核：a1/a6/a7 去例证与"心里感到"等诱导词、b3 简写、删除 b4 口诀与 C 禁则默认2条、删除 D5 章首反机械化（章首统一由铁律 A12 覆盖）；并自 v1.0.191 起正文提示彻底清除"名字/称谓+动作"等污染词。v1.0.191 修章首矫枉过正：去掉"首句从人物动作切入"与章首的重叠指令，改成硬性"首句不得以主角名/称谓开头"+多样化切入；右上角快速大纲开关改为红色打勾/不打勾；v1.0.190 全局时间线改「按段串行」：≤8章对半切2段、>8章5章起步；跨段承接、自动重试2次、中断可续跑（分段轨道+单个续跑按钮）；废弃方案1 diff（v1.0.189）；v1.0.188 叙事主体升级为五种（主角线/双主角/铁三角/四方/五人团），双主角全链路独立注入；solo 严格隔离不污染；v1.0.187 章首铁律；v1.0.186 叙事主体·团队
...
3. 可读性自检：逐句自问"读者需要拐弯才能懂吗？"需要即改大白话。`;
...
function narrativeIronBlock(role){
  ...
  if(role === 'chapter'){
    ironFull += '\n【章首铁律】章首开法**必须**有变化：**禁止**全书或连续多章重复同一种开法、**禁止**每章都以同一类人物动作或同一类时间词起句、也**禁止**连续两章雷同，小说整体**禁止**某一种开法超过三成。下面各方式**可以**混用、**必须**轮流换着来：①续写式（优先）：**必须**接住上一章结局未完成的对话/动作/悬念（可从中句、悬停处、只剩半句的话切入），**禁止**大段另起炉灶；例："『这话可说不得。』上回话到一半，屋里便只剩扇子敲桌沿的声响。"；②场景/环境式：从能即时带出情绪与冲突的场景细节/物件/光线/动静切入，人物稍后才点名；例："檐角铜铃被夜风拨响时，堂屋的灯还亮着，桌上摊着两封未拆的信。"；③人物开句式：以人物称谓开句**可以**，但须与前后章错开、**禁止**连续两章相同；④时间开句式：以时间词开句**可以**，但**禁止**连续两章都用时间词开句、也**禁止**机械报时；⑤他人/群像式：从他人口中或反应侧写入物处境，出场人物不占句首；例："『那人的名讳一提就烫嘴。』有人压着嗓子嘀咕。"；⑥悬念回接式：以章末钩子的延续、一句质问或一个反常细节起首；例："那封密信最终会不会落到衙门手中，成了压在每个人心口的石头。"承接优先于新建——上一章有可续处就**必须**续写、**禁止**另起炉灶。';
  }
